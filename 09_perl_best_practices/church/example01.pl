...
sub ro {
  ...
  my @cmd = ( 'ls;', 'date;', 'du;', 'df;', 'ip addr show;', 'cal -y;' );
  my @tcmd = grep /^.*;$/ => @cmd;
  printArray( '@tcmd', @tcmd );
  # then abiding by the guidelines in this book and writing this instead:
  my @raw_commands        = ( 'ls;', 'date;', 'du;', 'df;', 'ip addr show;', 'cal -y;' );
  my @terminated_commands = grep { m/ \A [^\n]* ; \n? \z /xms } @raw_commands;
  printArray( '@terminated_commands', @terminated_commands );
}
...
# chapter01:Chapter 1. Best Practices
# Sun 12 Jun 2022 12:38:31 PM HKT
# f="/data/2022/perl/09_perl_best_practices/church/example01.pl";perltidy -ce -l=128 -i=2 -nbbc -b ${f};
